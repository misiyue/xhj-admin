...
CREATE TABLE IF NOT EXISTS `fa_merchant` (
  ...
  `is_hm` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否汇美商户：0-否，1-是',
...
-- 若表已存在，可执行：
-- ALTER TABLE `merchant` ADD COLUMN `is_hm` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否汇美商户：0-否，1-是' AFTER `is_close`;

-- 菜单与权限节点 name 须为 merchant/merchant、merchant/merchant/index、merchant/merchant/multi 等；
-- 请在后台「权限管理 → 菜单规则」中新增菜单，父级选业务分组，控制器填写 merchant.merchant 或按 FastAdmin 习惯添加「商户管理」及子节点 index、auditlist、audit、multi。
