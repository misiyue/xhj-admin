...
require(['form'], function (Form) {
    ...
    Form.events.bindevent = function (form) {
        ...
        if (Config.simditor && $(Config.simditor.classname || '.editor', form).length > 0) {
            //修改上传的接口调用
            require(['upload', 'simditor', 'dompurify'], function (Upload, Simditor, DOMPurify) {
                ...
                DOMPurify.addHook('uponSanitizeElement', function (node, data, config) {
                    if (data.tagName === 'iframe') {
                        var allowedIframePrefixes = Config.simditor.allowiframeprefixs || [];
                ...
                $(Config.simditor.classname || '.editor', form).each(function () {
                    ...
                    editor = new Simditor({
                        ...
                        toolbar: (Config.simditor.toolbar && Config.simditor.toolbar.length) ? Config.simditor.toolbar : toolbar,
                        mobileToolbar: (Config.simditor.mobileToolbar && Config.simditor.mobileToolbar.length) ? Config.simditor.mobileToolbar : mobileToolbar,
